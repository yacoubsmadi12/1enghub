...
export const appRouter = router({
  ...
  dashboard: router({
    snapshot: protectedProcedure.query(async ({ ctx }) => {
      ...
      return { totalAssets: Number(total[0]?.value ?? 0), activeAssets: Number(active[0]?.value ?? 0), pendingApprovals: Number(pending[0]?.value ?? 0), hoursSaved: visibleAssets.reduce((sum, item) => sum + (item.estimatedHoursSaved || 0), 0), totalCostSaved: visibleAssets.reduce((sum, item) => sum + (item.estimatedCostSaved || 0), 0), impactAssets: visibleAssets.filter(item => (item.estimatedHoursSaved || 0) > 0 || (item.estimatedCostSaved || 0) > 0).length, teamScores, topContributors: Array.from(contributorMap.values()).sort((a, b) => b.score - a.score).slice(0, 6) };
    }),
  }),
  teams: router({
