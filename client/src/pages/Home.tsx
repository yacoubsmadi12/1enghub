...
export default function Home() {
  ...
  }).sort((a, b) => sortBy === "Name" ? a.name.localeCompare(b.name) : sortBy === "Most used" ? b.usage.localeCompare(a.usage) : 0), [activeFilter, query, sortBy]);
  ...
  return <div className="enghub-app">
    ...
    {notificationsOpen && <div className="notification-popover"><div className="flex items-center justify-between"><div><div className="eyebrow">Workspace inbox</div><h3 className="mt-1 text-sm font-semibold text-white">Notifications</h3></div><span className="queue-count">03</span></div><div className="notification-list"><div><span className="notification-bullet bg-amber-300" /><p><strong>IPRAN Runbook</strong> is waiting for Manager review.<small>1 hour ago</small></p></div><div><span className="notification-bullet bg-cyan-300" /><p><strong>NCE Configuration Tool</strong> has a new approved version.<small>2 hours ago</small></p></div><div><span className="notification-bullet bg-rose-300" /><p><strong>Network Change SOP</strong> needs requested changes.<small>Yesterday</small></p></div></div><button className="notification-footer" onClick={() => setNotificationsOpen(false)}>Mark all as read</button></div>}
