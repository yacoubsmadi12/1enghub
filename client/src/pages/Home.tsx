...
export default function Home() {
  ...
  const shareTeams = trpc.teams.available.useQuery(undefined, { enabled: Boolean(user), retry: false });
  const createShare = trpc.assets.share.useMutation({ onSuccess: () => { setShareNotice("Share created. The selected team can now access this governed asset."); }, onError: error => setShareNotice(error.message) });
  ...
  return <div className="enghub-app">
    ...
    {shareAsset && <div className="modal-backdrop" onClick={() => setShareAsset(null)}><div className="create-modal share-modal-polished" onClick={e => e.stopPropagation()}><button onClick={() => setShareAsset(null)} className="modal-close"><X size={17} /></button><div className="share-modal-orb"><Users2 size={21} /></div><div className="eyebrow mt-5"><Users2 size={13} /> Team access</div><h2 className="mt-2 text-2xl font-semibold text-white">Share {shareAsset.name}</h2><p className="mt-2 text-sm leading-6 text-slate-400">Share directly from Recent assets. The selected team receives governed access and the action is audited.</p><div className="share-form mt-6"><label>Recipient team<select value={shareTeamId} onChange={event => setShareTeamId(event.target.value)}><option value="">Select a team</option>{(shareTeams.data ?? []).map(team => <option key={team.id} value={team.id}>{team.name} · {team.code}</option>)}</select></label><label>Permission<select value={sharePermission} onChange={event => setSharePermission(event.target.value as typeof sharePermission)}><option value="view">View only</option><option value="download">View and download</option><option value="contribute">Contribute</option></select></label></div>{shareNotice && <p className="mt-4 rounded-lg border border-cyan-300/20 bg-cyan-300/10 p-3 text-xs leading-5 text-cyan-100">{shareNotice}</p>}<Button className="create-button mt-5 w-full" disabled={!shareTeamId || createShare.isPending} onClick={() => { if (!shareTeamId) return; createShare.mutate({ assetId: shareAsset.id, recipientType: "team", recipientTeamId: shareTeamId, permission: sharePermission }); }}>{createShare.isPending ? "Creating governed share..." : "Share with team"}<ArrowUpRight size={14} /></Button><p className="mt-3 text-center text-[11px] text-slate-500">Available for approved, published, or active assets.</p></div></div>}
