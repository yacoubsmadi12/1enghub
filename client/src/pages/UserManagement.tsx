...
import { Download, FileSpreadsheet, KeyRound, Plus, Save, Search, ShieldCheck, UploadCloud, UserRound, UsersRound, X } from "lucide-react";
...
type AdminUser = { id: string; username: string | null; employeeNumber: string | null; managerId: string | null; name: string | null; email: string | null; role: Role; isActive: boolean };
...
export default function UserManagement() {
  ...
  return <div className="min-h-screen bg-[#080f19] text-slate-200">
    <header className="border-b border-white/[0.07] bg-[#0b1421] px-6 py-4"><div className="mx-auto flex max-w-7xl items-center justify-between"><div className="flex items-center gap-3"><div className="brand-mark"><span>EH</span></div><div><div className="brand-name">ENGHUB</div><div className="text-[10px] text-slate-500">Governance console</div></div></div><Link href="/" className="text-xs text-cyan-300">Back to workspace</Link></div></header>
    <main className="mx-auto max-w-7xl px-6 py-10">
      ...
      <section className="panel-surface mt-6 overflow-hidden"><div className="flex flex-wrap items-center gap-3 border-b border-white/[0.07] px-5 py-4"><div className="relative min-w-[240px] flex-1"><Search size={14} className="absolute left-3 top-3 text-slate-500" /><input value={search} onChange={event => { setSearch(event.target.value); setPage(1); }} placeholder="Search employee number, name, username, or email..." className="h-9 w-full rounded-md border border-white/10 bg-[#0e1928] pl-9 pr-3 text-xs text-slate-200 outline-none" /></div><select value={roleFilter} onChange={event => { setRoleFilter(event.target.value as typeof roleFilter); setPage(1); }} className="h-9 rounded-md border border-white/10 bg-[#0e1928] px-3 text-xs text-slate-300"><option value="all">All roles</option><option value="top_manager">Top Manager</option><option value="manager">Manager</option><option value="team_member">Team Member</option></select><select value={statusFilter} onChange={event => { setStatusFilter(event.target.value as typeof statusFilter); setPage(1); }} className="h-9 rounded-md border border-white/10 bg-[#0e1928] px-3 text-xs text-slate-300"><option value="all">All statuses</option><option value="active">Active</option><option value="disabled">Disabled</option></select></div><div className="divide-y divide-white/[0.06]">{users.isLoading && <div className="p-8 text-center text-xs text-slate-500">Loading members...</div>}{users.isError && <div className="p-8 text-center text-xs text-rose-300">Unable to load members. Check PostgreSQL configuration.</div>}{visibleUsers.map(member => <ManagedUserRow key={member.id} member={member} managers={managers} teams={teams.data ?? []} onSave={(employeeNumber, managerId, name, email, role, isActive) => updateUser.mutate({ userId: member.id, employeeNumber: employeeNumber || undefined, managerId: managerId || undefined, name, email: email || undefined, role, isActive })} onReset={() => { const password = window.prompt(`New temporary password for ${member.username ?? member.name ?? "user"}`); if (password) resetPassword.mutate({ userId: member.id, temporaryPassword: password }); }} onToggle={() => setActive.mutate({ userId: member.id, isActive: !member.isActive })} onAssign={teamId => assignTeam.mutate({ userId: member.id, teamId, isPrimary: true })} />)}{!users.isLoading && visibleUsers.length === 0 && <div className="p-8 text-center text-xs text-slate-500">No users match your filters.</div>}</div><div className="flex items-center justify-between border-t border-white/[0.07] px-5 py-3 text-xs text-slate-500"><span>Showing {visibleUsers.length} of {filteredUsers.length}</span><div className="flex items-center gap-2"><Button variant="outline" size="sm" disabled={page <= 1} onClick={() => setPage(value => value - 1)}>Previous</Button><span>Page {page} / {pageCount}</span><Button variant="outline" size="sm" disabled={page >= pageCount} onClick={() => setPage(value => value + 1)}>Next</Button></div></div></section>
    </main>
  </div>;
}

function ManagedUserRow({ member, managers, teams, onSave, onReset, onToggle, onAssign }: { member: AdminUser; managers: AdminUser[]; teams: AdminTeam[]; onSave: (employeeNumber: string, managerId: string, name: string, email: string, role: Role, isActive: boolean) => void; onReset: () => void; onToggle: () => void; onAssign: (teamId: string) => void }) {
  ...
  return <div className="grid gap-4 px-5 py-4 xl:grid-cols-[1.15fr_1.25fr_1fr_1.15fr_1.05fr_auto] xl:items-center"><div className="flex items-center gap-3"><div className="profile-avatar">{(name || member.username || "U").slice(0, 2).toUpperCase()}</div><div><strong className="block text-sm text-slate-200">{member.username || "No username"}</strong><Badge className={member.isActive ? "border-emerald-400/20 bg-emerald-400/10 text-emerald-200" : "border-rose-400/20 bg-rose-400/10 text-rose-200"}>{member.isActive ? "ACTIVE" : "DISABLED"}</Badge></div></div><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-1"><input aria-label="Employee Number" value={employeeNumber} onChange={event => setEmployeeNumber(event.target.value)} placeholder="Employee Number" className="h-8 rounded-md border border-white/10 bg-[#0e1928] px-2 text-xs text-slate-200" /><input aria-label="Full name" value={name} onChange={event => setName(event.target.value)} placeholder="Full name" className="h-8 rounded-md border border-white/10 bg-[#0e1928] px-2 text-xs text-slate-200" /><input aria-label="Email" value={email} onChange={event => setEmail(event.target.value)} placeholder="Email" className="h-8 rounded-md border border-white/10 bg-[#0e1928] px-2 text-xs text-slate-200" /></div><select aria-label="Role" value={role} onChange={event => setRole(event.target.value as Role)} className="h-8 rounded-md border border-white/10 bg-[#0e1928] px-2 text-xs text-slate-300"><option value="top_manager">Top Manager</option><option value="manager">Manager</option><option value="team_member">Team Member</option></select><select aria-label="Direct manager" value={managerId} onChange={event => setManagerId(event.target.value)} className="h-8 rounded-md border border-white/10 bg-[#0e1928] px-2 text-xs text-slate-300"><option value="">No direct manager</option>{managers.filter(manager => manager.id !== member.id).map(manager => <option key={manager.id} value={manager.id}>{manager.name || manager.username}</option>)}</select><select aria-label="Team" defaultValue="" onChange={event => event.target.value && onAssign(event.target.value)} className="h-8 rounded-md border border-white/10 bg-[#0e1928] px-2 text-xs text-slate-300"><option value="">Assign team...</option>{teams.map(team => <option key={team.id} value={team.id}>{team.name}</option>)}</select><div className="flex flex-wrap gap-2 xl:justify-end"><Button size="sm" variant="outline" title="Save profile, role, and reporting line" onClick={() => onSave(employeeNumber, managerId, name, email, role, member.isActive)}><Save size={13} /></Button><Button size="sm" variant="outline" title="Reset password" onClick={onReset}><KeyRound size={13} /></Button><Button size="sm" variant="outline" onClick={onToggle}>{member.isActive ? "Disable" : "Activate"}</Button></div></div>;
}
