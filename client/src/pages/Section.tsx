...
export default function Section() {
  ...
  const smtpQuery = trpc.administration.getSmtpSettings.useQuery(undefined, { enabled: user?.role === "top_manager", retry: false });
  const saveSmtp = trpc.administration.saveSmtpSettings.useMutation({ onSuccess: () => { setSmtpNotice("SMTP settings saved securely."); setSmtpPassword(""); void smtpQuery.refetch(); }, onError: error => setSmtpNotice(error.message) });
  ...
  const renderContent = () => {
    ...
    if (section === "settings") return <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]"><div className="rounded-xl border border-cyan-300/15 bg-gradient-to-br from-cyan-300/[0.08] to-white/[0.02] p-6"><div className="eyebrow">Mail delivery</div><h2 className="mt-2 text-xl font-semibold text-white">SMTP email server</h2><p className="mt-2 text-xs leading-5 text-slate-400">Configure the server used for secure password reset emails. The password is encrypted before it is stored.</p><div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="field-label">SMTP host<input value={smtpHost || smtpQuery.data?.host || ""} onChange={event => setSmtpHost(event.target.value)} placeholder="smtp.company.com" /></label><label className="field-label">Port<input type="number" value={smtpPort || String(smtpQuery.data?.port || 587)} onChange={event => setSmtpPort(event.target.value)} /></label><label className="field-label">Username<input value={smtpUsername || smtpQuery.data?.username || ""} onChange={event => setSmtpUsername(event.target.value)} placeholder="mailer@company.com" /></label><label className="field-label">From email<input type="email" value={smtpFrom || smtpQuery.data?.fromEmail || ""} onChange={event => setSmtpFrom(event.target.value)} placeholder="ENGHUB <enghub@company.com>" /></label><label className="field-label sm:col-span-2">SMTP password<input type="password" value={smtpPassword} onChange={event => setSmtpPassword(event.target.value)} placeholder={smtpQuery.data ? "Leave empty to keep current password" : "Required"} /></label></div><label className="mt-4 flex items-center gap-2 text-xs text-slate-400"><input type="checkbox" checked={smtpSecure} onChange={event => setSmtpSecure(event.target.checked)} /> Use secure TLS connection</label><Button className="create-button mt-6" disabled={saveSmtp.isPending} onClick={() => saveSmtp.mutate({ host: smtpHost || smtpQuery.data?.host || "", port: Number(smtpPort || smtpQuery.data?.port || 587), secure: smtpSecure || Boolean(smtpQuery.data?.secure), username: smtpUsername || smtpQuery.data?.username || "", password: smtpPassword || undefined, fromEmail: smtpFrom || smtpQuery.data?.fromEmail || "" })}>{saveSmtp.isPending ? "Saving..." : "Save SMTP settings"}</Button>{smtpNotice && <p className="mt-4 text-xs text-cyan-200">{smtpNotice}</p>}</div><div className="rounded-xl border border-white/[0.07] bg-white/[0.03] p-6"><div className="eyebrow">Recovery policy</div><h2 className="mt-2 text-xl font-semibold text-white">Password reset flow</h2><div className="mt-5 space-y-4 text-sm leading-6 text-slate-400"><p>Users request a one-time reset link using their username or company email.</p><p>Links expire after 30 minutes and passwords are hashed server-side.</p><p>The system returns a neutral response so account existence is not exposed.</p></div></div></div>;
